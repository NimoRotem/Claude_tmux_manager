...
try:
    # OPS lets the panel fill the target from the register instead of asking a
    # human to retype a title. Optional: without it the tab still works, it just
    # cannot auto-fill, and a mistyped title is exactly what gets a filing
    # bounced or, worse, filed against the wrong application.
    import epo_ops
...
@router.get("/api/epobs/resolve")
async def api_epobs_resolve(number: str = ""):
    """Look a target up in the European Patent Register."""
    if epo_ops is None:
        return _err("OPS client not installed on this host", 501)
    if not (number or "").strip():
        return _err("give a number", 400)
    try:
        pub, _kind = epo_ops.normalise(number)
        parsed = epo_ops.parse_biblio(epo_ops.biblio(pub))
        b = epo_ops.grant_publication(parsed)
        cl = epo_ops.claims(pub)
        feats = (epo_ops.split_features(max(cl["claims"], key=len))
                 if cl.get("available") else [])
        return JSONResponse({
            "publication": pub, "application": parsed.get("application"),
            "title": parsed.get("title"), "proprietors": parsed.get("proprietors"),
            "granted": bool(b), "grant_kind": (b or {}).get("kind"),
            "grant_date": (b or {}).get("date"),
            "claim_count": len(cl.get("claims") or []),
            "features": feats,
        })
    except Exception as exc:                                      # noqa: BLE001
        return _err("lookup failed: %s" % exc, 502)
